...
players_dic = {  # Move the dictionary definition here
    player_name: [],
    dealer_name: [],
}
...
def deal_initial_hands(player, dealer):
    deal_card(player)
    deal_card(player)
    deal_card(dealer)
...
def play_game(players_dic):
    PLAYER = players_dic[player_name]
    DEALER = players_dic[dealer_name]

    deal_initial_hands(PLAYER, DEALER)
...
def main():
    ...
    while play_again:
        print(logo)
        play_game(players_dic)  # Pass the players_dic to the play_game function

        new_blackjack_game = input("Would you like to play another game of Blackjack? Type 'y' or 'n': ").lower()
        if new_blackjack_game != "y":
            play_again = False
